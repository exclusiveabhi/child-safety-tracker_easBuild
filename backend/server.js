...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    const existingUser = await User.findOne({ busNumber });
    if (existingUser) {
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user && await bcrypt.compare(password, user.password)) {
      const token = jwt.sign({ bus: user.busNumber, route: user.route }, 'secret_key');
...
// API to fetch students by bus number
app.get('/students/:busNumber', authenticate, async (req, res) => {
...
app.post('/bus-location', authenticate, async (req, res) => {
